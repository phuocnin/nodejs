...
let upfile = (req, res) => {
    ...
    con.connect(function (x) {
        ...
        if (req.fileValidationError) {
            return res.send(req.fileValidationError + `<hr href="./upload-file">Upload another image</hr>`);
        }
        else if (!req.file) {
            return res.send('Please select an image to upload' + `<div><a href="./upload-file">Upload another image</a></div>`);
        }
